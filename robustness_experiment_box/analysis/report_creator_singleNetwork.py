...
class ReportCreator:
    ...
    def create_hist_figure(self) -> plt.Figure:
        hist_plot = sns.histplot(data=self.df, x="epsilon_value")
        figure = hist_plot.get_figure()
        plt.close()
        return figure

    def create_box_figure(self) -> plt.Figure:
        # For a single network, just show the distribution
        box_plot = sns.boxplot(y=self.df["epsilon_value"])
        box_plot.set_xticklabels(["epsilon_value"])
        figure = box_plot.get_figure()
        plt.close()
        return figure

    def create_kde_figure(self) -> plt.Figure:
        kde_plot = sns.kdeplot(data=self.df, x="epsilon_value", fill=True)
        figure = kde_plot.get_figure()
        plt.close()
        return figure

    def create_ecdf_figure(self) -> plt.Figure:
        ecdf_plot = sns.ecdfplot(data=self.df, x="epsilon_value")
        figure = ecdf_plot.get_figure()
        plt.close()
        return figure

    def create_anneplot(self):
        df = self.df.sort_values(by="epsilon_value")
        cdf_x = np.linspace(0, 1, len(df))
        plt.plot(df.epsilon_value, cdf_x, label="network")
        if "smallest_sat_value" in df.columns:
